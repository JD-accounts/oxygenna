    ...
    function InboxController($scope, $filter, $location, $state, $mdMedia, $mdBottomSheet, $stateParams, $mdDialog, $mdToast, emails, contacts) {
        ...
        function createEmailGroups() {
            ...
            if(emails.status === 200) {
                ...
                vm.emailGroups = [{
                    name: $filter('triTranslate')('EMAIL.INBOX.GROUPS.TODAY'),
                    from: moment().startOf('day'),
                    to: moment().endOf('day')
                },{
                    name: $filter('triTranslate')('EMAIL.INBOX.GROUPS.YESTERDAY'),
                    from: moment().subtract(1, 'days').startOf('day'),
                    to: moment().subtract(1, 'days').endOf('day')
                },{
                    name: $filter('triTranslate')('EMAIL.INBOX.GROUPS.OLDER'),
        ...
        function composeClick($event) {
            $mdDialog.show({
                ...
                targetEvent: $event,
                locals: {
                    title: $filter('triTranslate')('EMAIL.NEW'),
                    email: {
                        to: [],
                        cc: [],
                        bcc:[],
        ...
        function sendEmail($event, email) {
            ...
            $mdToast.show(
                $mdToast.simple()
                .content($filter('triTranslate')('EMAIL.SENT', {to: sentTo.join(', ')}))
                .position('bottom right')
                .hideDelay(3000)
            );
        }
        ...
        $scope.$on('deleteEmail', function($event, deleteEmail) {
            angular.forEach(vm.emailGroups, function(group) {
                ...
                if(null !== removeEmailIndex) {
                    ...
                    $mdToast.show(
                        $mdToast.simple()
                        .content($filter('triTranslate')('EMAIL.DELETED'))
                        .position('bottom right')
                        .hideDelay(3000)
                    );
                }
